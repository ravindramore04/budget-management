...
function setAction(code,id){
    ...
    switch(code){
       ...
       // alert(strgiId);
            ...
            break;
        case 1:
            document.rptParam.action = "<%=strPath+"POViewRpt.do"%>";
            break;
...
<!-- colspan="2"  -->
      ...
      <% int indx=-1;
	     if(hmData!=null && hmData.size()>0){
			...
			out.println("--hmFinal---"+hmFinal.size());	
           
				if(hmFinal!=null && hmFinal.size()>0){
				out.println("size inside if"+hmFinal.size());
		   int outerloop=hmFinal.size()/3;//if( outerloop==0) {outerloop++;}
		   for(int outerindx=0;outerindx<=outerloop;outerindx++)
		   {indx++;
	%>
	 <tr bgcolor="<%=strCol2%>"> 
				<% 
				//out.println("--strId--"+hmData);
                for(;indx<hmFinal.size();indx++){
                    HashMap hmt1 = (HashMap)hmFinal.get(""+indx);
					//out.println("--strId--"+hmt);
                    if(hmt1!=null && hmt1.size()>0){
                        String strId = (String)hmt1.get("HeadId");
						//out.println("--strId--"+strId);
                        String strName = (String)hmt1.get("strName");
                        %>
