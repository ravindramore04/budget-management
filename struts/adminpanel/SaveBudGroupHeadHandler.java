								 ...
								 HttpServletResponse response) throws RuntimeException,Exception
	{
		...
		try{
			...
			if(nItr>0){
				...
				String strRemark=(String)daf.get("strGroupType");
				...
				if(strId == null || strId.length()==0){
					...
					cvdal.setSQL("checkGroupHead",vec);
					...
					if(vec12 != null && vec12.size()!=0){
						...
						if(hmt!=null && hmt.size()>0){
							String strNm = (String)hmt.get("strBudgroupNm");
				...
				if(bDuplicate){
					...
					if(strId == null || strId.length()==0){
						...
						int iNewId = 100001;
						vec.clear();
						cvdal.setSQL("lastGroupHeadID",vec);
					...
					if(strId == null || strId.length()==0){
						//insert
						cvdal.setSQL("insertGroupHead",vec);
					}else{
						//update record
						vec.addElement(strId);
						cvdal.setSQL("updateGroupHead",vec);
					}
