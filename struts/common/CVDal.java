...
public class CVDal
{
	...
	public  static void setALLSQL()
	{
		...
		ALLSQL.put("openAllHeadWithBalanceAllDept","select a.AllocId, a.HeadId, a.dblAmount, b.strName, d.strDepartmentNm, a.dblReservedAmount, a.dblUtilisedAmount from budgetalloc a, budgethead b, departments d where a.HeadId = b.HeadId and a.strDepartmentId = d.strDepartmentId  order by d.strDepartmentNm");
...
//		ALLSQL.put("ViewAllVouch","select budgethead.HeadId, sum(voucher.dblAmount) as VSUM from budgethead,voucher where budgethead.HeadId=voucher.HeadId and  voucher.Dt between '?' and '?' group by budgethead.HeadId order by budgethead.HeadId where budgethead.HeadId='?'");
		...
		ALLSQL.put("openAllVoucherSearch","select v.voucher_id,v.voucher_number,b.budget_note_id,b.ponumber,v.receiver_name,v.voucher_date,d.strDepartmentNm,bh.strName,v.amount+v.tds_amount as amt from voucher_details v left join budget_note b on  v.budget_note_id = b.budget_note_id  left join  budgetalloc ba on  b.AllocId = ba.AllocId left join BudgetHead bh on ba.HeadId = bh.HeadId left join departments d on ba.strDepartmentId=d.strDepartmentId where v.receiver_name like '%?%' order by v.voucher_number desc;");
		ALLSQL.put("openAllVoucherSearchWL","select v.voucher_id,v.voucher_number,b.budget_note_id,b.ponumber,v.receiver_name,v.voucher_date,d.strDepartmentNm,bh.strName,v.amount+v.tds_amount as amt from voucher_details v left join budget_note b on  v.budget_note_id = b.budget_note_id  left join  budgetalloc ba on  b.AllocId = ba.AllocId left join BudgetHead bh on ba.HeadId = bh.HeadId left join departments d on ba.strDepartmentId=d.strDepartmentId where v.receiver_name like '%?%' order by v.voucher_number desc limit ?,?");
